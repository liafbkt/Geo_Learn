...
- 2026-09-08 `v0.1.0-rc.8` 的真实 Windows run `34225148188` 已验证上述 LF 修复：notices freshness 通过，QG-01..09、门禁证据和签名预检也通过。随后 `pnpm tauri build … --ci -- --locked` 使 pnpm 吞掉 Tauri-to-Cargo 的分隔符，Tauri 收到孤立 `--locked` 并在编译/签名/上传前退出；没有 Release 资产。人为尝试的 `pnpm run tauri -- …` 在本地验证中同样错误地将首个 `--` 传给 Tauri，因此未推送。
- 2026-09-08 `v0.1.0-rc.9` 的真实 Windows run `34232561369`：QG-01..09、证据写入、notices freshness 和签名预检均通过，但 `pnpm exec tauri build … --ci -- --locked` 仍让 pnpm 吞掉第一层分隔符，Tauri 在任何编译、签名或上传前拒绝孤立的 `--locked`；没有 Release 或资产。最终修复不再跨三层转发该 Cargo 参数：工作流改为 `pnpm tauri build … --ci`，并在构建后、验签前用 `git diff --exit-code -- src-tauri/Cargo.lock` 证明依赖图未变化。发布预检还会比较 `pnpm-lock.yaml` 中 `@tauri-apps/plugin-updater` 与 `Cargo.lock` 中 `tauri-plugin-updater` 的 major.minor；2.10/2.11 不一致会在构建前失败。真实本地探针已越过两种 Tauri 错误并执行前端构建，随后仅因本机 MSVC `link.exe` 缺失停止；下一候选只能由 GitHub Windows runner 验证。
- 2026-09-08 `v0.1.0-rc.10` 的真实 Windows run `34236471577`：在 QG-01 前的 identity-only preflight 失败，原因是 `lib.mjs` 静态导入了仅在后续 `pnpm install` 才会安装的 `yaml` 包；没有质量门禁、签名或草稿资产运行。修复移除该外部导入，只用 Node 内建正则读取 pnpm root importer，并新增无 `node_modules` 隔离预检回归；90 项版本/发布测试通过。`rc.10` 保留为失败候选，必须用新 tag 重测。

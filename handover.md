...
> 更新：2026-08-27；分支：`codex/user-test-mvp`；阶段：Tasks 1–2 已完成并通过独立审查，下一步 Task 3。
...
| Tasks 3–16 | 未开始 | 下一步 Task 3，按依赖顺序执行 |
...
- Task 2 最终证据：完整 Vitest 2 文件/12 测试、`pnpm typecheck`、`pnpm lint`、`pnpm build`；详见上述 report。
...
1. Task 3：资源加载、无效包隔离、内容迁移、窄 Tauri 资源命令。
2. Task 4：掌握阶段与易忘；Task 5：答案/提示/题目；Task 6：调度与摸底。
3. Task 7：repository 与 SQLite 事务；Task 8：备份合并/替换。
4. Tasks 9–13：地图、练习 UI、首页/探索/总结，形成浏览器可完整试用闭环。
5. Tasks 14–16：内容管线、三包数据、E2E 与个人安装包；原生安装包仍取决于 MSVC 授权。
